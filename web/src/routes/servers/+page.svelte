<script lang="ts">
  ...
  import { Plus, Search, Server, Star, MoreVertical, Eye, Pencil, Plug, ArrowRightLeft, GitCompare, ShieldCheck, ShieldAlert } from 'lucide-svelte';
  import {
    fetchServers,
    serverStore,
    setFilterFavorites,
        ...
        <table class="min-w-full divide-y divide-slate-200">
          <thead class="bg-slate-50">
            <tr>
              ...
              <th scope="col" class="hidden xl:table-cell px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-slate-500">
                Status
              </th>
              <th scope="col" class="hidden lg:table-cell px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-slate-500">
                Labels
          ...
          <tbody class="divide-y divide-slate-100 bg-white">
            {#each $serverStore.filteredServers as server}
              <tr class="hover:bg-slate-50">
                ...
                <td class="hidden xl:table-cell whitespace-nowrap px-4 py-4 align-top">
                  ...
                  {:else if server.credentialStatus === 'invalid' || server.credentialStatus === 'expired' || server.credentialStatus === 'locked'}
                    <span class="inline-flex items-center gap-1 text-xs font-medium text-rose-600">
                      <ShieldAlert size={14} /> {server.credentialStatus}
                    </span>
                  {:else}
